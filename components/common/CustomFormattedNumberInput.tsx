...
const CustomFormattedNumberInput: React.FC<InputProps<any>> = ({
  name,
  type,
  label,
  placeholder,
  ...
  useEffect(() => {
    if (placeholder?.includes("Contact")) {
      let formattedNumber = formatPhoneNumber(watch(name));
      formattedNumber && setValue(name, formattedNumber);
    }
  }, [watch(name)]);
...
export default CustomFormattedNumberInput;
