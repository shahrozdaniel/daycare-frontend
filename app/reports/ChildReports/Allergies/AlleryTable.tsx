...
interface AllergyTableProps<TData, TValue> {
...
export function AllergyTable<TData, TValue>({
  columns,
  data,
  dataToExport,
  selectedOption,
  selectedReport,
  show
}: AllergyTableProps<TData, TValue>) {
    ...
    <>
      ...
      <div className="flex md:flex-col lg:flex-row justify-between">
        ...
        <span className="text-grey-placeholder md:text-sm lg:text-md md:w-20 lg:w-max ">
          ...
          {selectedOption === 'Child Reports' && selectedReport === 'Allergies' && !show && <>
            ...
            <div className="relative">
              <Input
                placeholder="Allergy"
                value={
                  (table.getColumn("alergeyName")?.getFilterValue() as string) ?? ""
                }
                onChange={(event) =>
                  table.getColumn("alergeyName")?.setFilterValue(event.target.value)
                }
                className="md:w-[160px] lg:w-[180px] rounded-none ml-2 border-grey-border1 bg-grey-border1 text-grey-placeholder placeholder:text-grey-placeholder"
              />
              <Image
